...
//#define N 100
//
////Структура, которая описывает координаты точки
//struct Point
//{
//	double x;
//	double y;
...
//};
//
////На вход принимает символьный массив, которую проверяет на допустимые символы и возвращает 0, если присутствуют недопустимые символы, иначе возвращает 1
...
////Принимает координаты 2-х вершин треугольника и координаты центра окружности, и по математической формуле находит и возвращает координаты точки, в которую падает высота из центра окружности
...
////Принимает координаты вершин треугольника и координаты центра окружности и возвращает 1, если центр окружности находится внутри треугольника, иначе 0
//int IsCenterInTriangle(struct Triangle ABC, struct Point O); 
////Принимает координаты вершин треугольника и координаты центра окружности с его радиусом, возвращает 1, если вершина треугольника находится внутри круга
//int IsPeakInCircle(struct Triangle ABC, struct Point O, double r); 
////Принимает координаты вершин треугольника и координаты центра окружности с его радиусом, возвращает 1, если вершина треугольника находится на круге
//int IsPeakOnCircle(struct Triangle ABC, struct Point O, double r); 
...
////Принимает координаты вершин треугольника и координаты центра окружности с его радиусом, возвращает 1, если одна из сторон треугольника пересекает круг, а другие две не пересекают
//int IsTriangleOnCircle(struct Triangle ABC, struct Point O, double r); 
////Принимает координаты 2-х вершин треугольника и координаты центра окружности с его радиусом, возвращает 1, если прямая касается круга
//int IsLineTouching(struct Point A, struct Point B, struct Point O, double r);
...
//int AreLinesTouching(struct Triangle ABC, struct Point O, double r);
////Принимает координаты вершин треугольника и координаты центра окружности с его радиусом, вызывает проверки на наличие общих точек между треугольником и кругом
...
//int IsNumber(char str[])
//{
//	if (str[0] < '1' || str[0] > '9') return 0; //является ли первый символ натуральным числом
...
//	int AB = IsLineTouching(ABC.A, ABC.B, O, r);
//	int BC = IsLineTouching(ABC.B, ABC.C, O, r);
//	int CA = IsLineTouching(ABC.C, ABC.A, O, r);
//
...
//void FindCommonPoints(struct Triangle ABC, struct Point O, double r)
//{
//	if (IsPeakOnCircle(ABC, O, r) == 1) return; //проверка находятся ли вершины на круге
//
//	if (IsPeakInCircle(ABC, O, r) == 1) return; //проверка находятся ли вершины в круге
//
//	if (IsCenterInTriangle(ABC, O) == 1) return; //проверка находится ли центр окружности в треугольнике
//
//	if (IsTriangleOnCircle(ABC, O, r) == 1) return; //проверка находится ли треугольник в круге
//
//	if (AreLinesTouching(ABC, O, r) == 1) return; //проверка на касательные
//
//	puts("\nНет общих точек");
//}